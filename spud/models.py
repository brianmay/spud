...
class photo(base_model):
    ...
    name = models.CharField(max_length=384, blank=True, db_index=True)
    path = models.CharField(max_length=765, blank=True, db_index=True)
    ...
    view = models.CharField(max_length=192, blank=True)
    rating = models.FloatField(null=True, blank=True, db_index=True)
    description = models.TextField(blank=True)
    timezone = models.CharField(max_length=100)
    datetime = models.DateTimeField(db_index=True)
    camera_make = models.CharField(max_length=96, blank=True)
    camera_model = models.CharField(max_length=96, blank=True)
    flash_used = models.CharField(max_length=3, blank=True)
    focal_length = models.CharField(max_length=192, blank=True)
    exposure = models.CharField(max_length=192, blank=True)
    compression = models.CharField(max_length=192, blank=True)
    aperture = models.CharField(max_length=48, blank=True)
    level = models.IntegerField()
    iso_equiv = models.CharField(max_length=24, blank=True)
    metering_mode = models.CharField(max_length=48, blank=True)
    focus_dist = models.CharField(max_length=48, blank=True)
    ccd_width = models.CharField(max_length=48, blank=True)
    ...
    def queers(self):
        return queer.objects.all()

    def update_from_queer(self, queer):
        if queer.update_title:
            self.title = queer.update_title

        if queer.update_photographer:
            self.photographer = queer.update_photographer

        if queer.update_location:
            self.location = queer.update_location

        if queer.update_view:
            self.view = queer.update_view

        if queer.update_description:
            self.description = queer.update_description

        if queer.update_timezone:
            self.timezone = queer.update_timezone

        if queer.update_comment:
            self.comment = queer.update_comment

        if queer.update_status:
            self.status = queer.update_status

        for album in queer.add_albums.all():
            photo_album.objects.get_or_create(photo=self,album=album)

        for album in queer.delete_albums.all():
            photo_album.objects.filter(photo=self,album=album).delete()

        for category in queer.add_categorys.all():
            photo_category.objects.get_or_create(photo=self,category=category)

        for category in queer.delete_categorys.all():
            photo_category.objects.filter(photo=self,category=category).delete()

        for person in queer.add_persons.all():
            # try to guess the position we should assign
            pp = photo_person.objects.filter(photo=self).order_by("-position")
            try:
            ...
            photo_person.objects.get_or_create(photo=self,person=person,
...
class queer(base_model):
    name = models.CharField(max_length=20)
    update_title = models.CharField(max_length=192, blank=True)
    update_photographer = models.ForeignKey(person, null=True, blank=True, related_name='queer_updates')
    update_location = models.ForeignKey(place, null=True, blank=True, related_name='queer_updates')
    update_view = models.CharField(max_length=192, blank=True)
    update_description = models.TextField(blank=True)
    update_timezone = models.CharField(max_length=100, blank=True)
    update_comment = models.TextField(blank=True)
    update_status = models.CharField(max_length=1, blank=True, choices=PHOTO_STATUS)
    add_albums = models.ManyToManyField(album, related_name='queer_adds')
    delete_albums = models.ManyToManyField(album, related_name='queer_deletes')
    add_categorys = models.ManyToManyField(category, related_name='queer_adds')
    delete_categorys = models.ManyToManyField(category, related_name='queer_deletes')
    add_persons = models.ManyToManyField(person, related_name='queer_adds')
    delete_persons = models.ManyToManyField(person, related_name='queer_deletes')
    rotate = models.CharField(max_length=4, null=True, blank=True, choices=PHOTO_ROTATE)
    ...
    @models.permalink
    def get_absolute_url(self):
        return('queer_detail', [ str(self.pk) ])

    @models.permalink
    def get_edit_url(self):
        return('queer_edit', [ str(self.pk) ])

    @models.permalink
    def get_delete_url(self):
        return('queer_delete', [ str(self.pk) ])

    def get_breadcrumbs(self):
        breadcrumbs = []
        breadcrumbs.append(breadcrumb(reverse("spud_root"),"home"))
        breadcrumbs.append(breadcrumb(reverse("queer_list"),"queers"))
        breadcrumbs.append(breadcrumb(self.get_edit_url(),self))
        return breadcrumbs
